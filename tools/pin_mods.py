...
def _google_drive_asset(source: dict[str, Any]) -> tuple[str, str, bytes]:
    from ichalaunch.core.process import google_drive_url

    file_id = str(source.get("id") or "")
    if not file_id:
        raise SystemExit("google_drive source has no id")
    name = str(source.get("filename") or file_id)
    return "-", name, _get(google_drive_url(file_id))
...
def resolve(source: dict[str, Any]) -> tuple[str, str, bytes]:
    ...
    if stype == "google_drive":
        return _google_drive_asset(source)
...
def _iter_sources(mod: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    ...
    if isinstance(source, dict):
        out.append((mod_id, source))
    addon = mod.get("addon_source")
    if isinstance(addon, dict):
        out.append((f"{mod_id}:addon_source", addon))
    return out
...
def main() -> int:
    ...
    for mod in catalog:
        ...
        for label, source in _iter_sources(mod):
            ...
            print(f"  {label:24s} {state:12s} {name} ({tag})")
            if pinned:
