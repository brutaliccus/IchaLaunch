...
class HomePage(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.summary.body.addLayout(self.summary_host)

        root.addWidget(self.logo)
        root.addWidget(sub)
        root.addWidget(self.status)
        root.addWidget(self.loading_wrap)
    ...
    def _load_logo(self) -> None:
        ...
        if path.exists():
            ...
            if not pix.isNull():
                scaled = pix.scaledToWidth(300, Qt.TransformationMode.SmoothTransformation)
        ...
        if installed:
            ...
            if not enabled_ids:
                ...
                for cat in cats:
                    ...
                    cat_lbl = QLabel(cat)
                    cat_lbl.setStyleSheet("color: #a0a0a8; font-size: 12px; font-weight: 600;")
                    self.summary_host.addWidget(cat_lbl)
                    self.summary_host.addWidget(FlowChips(names))

            launch = QLabel(
                f"Launch via: {'VanillaFixes.exe' if settings.get('vanillafixes_enabled') else 'WoW.exe'}"
            )
            launch.setObjectName("Muted")
            self.summary_host.addWidget(launch)
        else:
            self.status.setText("Client not found — use INSTALL (bottom right) or Settings → Browse")
            tip = QLabel(
